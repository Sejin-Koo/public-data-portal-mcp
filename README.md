...
종목코드·상호 이력·대표자·소재지·업종)를 한 묶음으로 돌려줍니다. 해석 경로는 2단계입니다.
...
★ **두 파일을 하나로 합치지 마세요.** 1번은 매주 갈아끼우는 스냅샷이라, 누적이 필요한
...
★ 휴업·폐업 상태는 색인에 담지 않습니다. 색인에 굳혀 두면 한 달 내내 낡은 값을 답하게
되므로, 현재 상태는 국세청 사업자등록 상태조회로 조회 시점에 확인하세요.
