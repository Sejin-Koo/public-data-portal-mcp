...
async function callOp(op, params, { retries = 1 } = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // ★ 429(Too Many Requests)는 일시적 오류가 아니라 **요청한도 소진**이다. 재시도하면
      //   한도만 더 소모하므로 즉시 전용 코드로 돌려보낸다. 실측(2026-09-01): 원 API는
      //   오퍼레이션(업무구분)별로 한도를 걸어, 같은 시각에 용역은 429인데 공사는 정상이었다.
      if (status === 429) {
        return {
          ok: false,
          nodata: false,
          ...
          totalCount: 0,
          code: "RATELIMIT",
          msg: "HTTP 429 Too Many Requests — 이 업무구분의 요청한도가 소진되었습니다.",
        };
      }
      if (status !== 200) {
        lastErr = `HTTP ${status}`;
...
export async function searchScsbidWinners({
  ...
  companyName,
  limit = 50,
  // ★ 한 페이지에 담는 건수. 원 API의 일일 한도는 **오퍼레이션당 1,000회**이고 그 단위가
  //   "호출 횟수"이므로, 페이지를 크게 받을수록 같은 데이터를 적은 호출로 얻는다.
  //   실측(2026-09-01, 공사 기본형·같은 40초): 100 → 500건 / 500 → 2,000건 / 999 → 3,996건.
  //   호출당 소요 시간은 거의 같아 페이지 크기가 그대로 수집량이 된다. 결과 동일성도 확인했다
  //   (서울교통공사 3월 공사: 100·999 모두 65건 36,025,790,239원, 행 집합 차집합 0).
  //   페이지를 덜 도는 만큼 시간 상한에도 덜 걸린다.
  rowsPerPage = 999,
  summaryOnly = false,
  topN = 10,
} = {}) {
  ...
  for (const t of types) {
      ...
      while (true) {
        ...
        if (!r.ok) {
          ...
          if (!r.nodata) {
            ...
            notes.push(label);
            truncated = true;
            // ★ 요청한도 소진은 남은 창·페이지를 더 두드려봐야 전부 같은 결과다.
            //   계속 호출하면 한도만 더 소모하므로 즉시 전체 수집을 중단한다.
    ...
    if (timeBudgetHit || rateLimited || rows.length >= rowCap) break;
  }
  ...
  if (rateLimited) {
    caveats.push(
      `★★ **원 API의 요청한도가 소진되었습니다(HTTP 429).** 조회를 중단했으므로 이 결과는 ` +
        `요청 기간 전체가 아닙니다 — 건수·합계·순위를 내지 마세요. ` +
        `**같은 조건으로 재시도하지 마세요.** 일시적 오류가 아니라 한도 소진이라 재시도는 ` +
        `한도만 더 소모하며, 대개 익일 초기화됩니다. ` +
        `★ 한도는 **오퍼레이션당 일일 1,000회**입니다(공공데이터포털 활용신청 상세 기준). ` +
        `응답의 upstream호출수로 각 호출이 얼마를 썼는지 확인할 수 있습니다. ` +
        `★ 한도는 **업무구분(오퍼레이션)별로 따로** 걸립니다 — 실측(2026-09-01): 같은 시각에 ` +
        `용역은 429인데 공사는 정상 조회됐습니다. 다른 업무구분이 필요한 작업이면 그쪽은 ` +
        `계속 조회할 수 있습니다. 소진된 구간: ${windowFailures.join(" / ")}`
    );
  } else if (windowFailures.length) {
    caveats.push(
      `★ 조회 구간을 나눈 창 중 ${windowFailures.length}개가 **호출에 실패해 그 구간이 통째로 빠졌습니다** — 이 결과는 요청 기간 전체가 아닙니다. 실패 구간: ${windowFailures.join(
        " / "
      )}. 건수·합계·순위를 내지 말고, 실패한 구간만 따로(1개월 이내로) 다시 호출해 합치세요. resultCode 07은 조회기간 상한 초과, ERR은 네트워크·타임아웃, RATELIMIT은 요청한도 소진입니다.`
    );
  }
  if (timeBudgetHit) {
    caveats.push(
  ...
  return {
    ...
    // ★ 이 호출이 원 API를 몇 번 두드렸는지. 일일 한도가 오퍼레이션당 1,000회이므로
    //   호출자가 이 값을 누적해 남은 여유를 가늠할 수 있다.
    upstream호출수: calls,
    수집건수: rows.length,
    ...
    ...(timeBudgetHit
      ...
      : rateLimited
      ? { 잘림사유: "요청한도초과", 실패구간: windowFailures }
