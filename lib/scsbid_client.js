...
export async function searchScsbidWinners({
  ...
  if (bidNtceNo) {
    ...
    for (const t of types) {
      ...
      if (r.ok) rows.push(...asArray(r.items).map((x) => normalizeWinnerRow(x, t)));
      else if (!r.nodata && r.code !== "ERR") notes.push(`${t}: ${r.code} ${r.msg ?? ""}`.trim());
    }
    return {
      조회기준: "입찰공고번호",
      공고번호: bidNtceNo,
  ...
  for (const t of types) {
    ...
    for (const w of windows) {
      ...
      while (true) {
        ...
        const batch = asArray(r.items).map((x) => normalizeWinnerRow(x, t));
        rows.push(...batch);
        perType[t].수집 += batch.length;
  ...
  if (windows.length > 1) {
    notes.push(`조회구간을 ${RANGE_LIMIT_DAYS}일 창 ${windows.length}개로 나눠 호출했습니다.`);
  }
  if (!usesSearch) {
    notes.push(
