...
async function fetchYearAll(year) {
  ...
  // ★ numOfRows=5000 단일 호출이라, 어느 해가 5,000건을 넘으면 조용히 잘린다. 종전에는 total을
  //   읽어두고도 items.length와 대조하지 않아 잘림이 드러나지 않았다. 이름 필터는 받아온
  //   범위 안에서만 도는 것이므로, 잘린 채로 "해당 가맹본부 없음"이 나올 수 있다.
  const truncated = total > items.length;
  const out = { items, total, truncated };
...
export async function searchFranchiseDisclosure({
  ...
  let truncatedYearList = null;

  if (filtering) {
    const all = await fetchYearAll(year);
    total = all.total;
    if (all.truncated) truncatedYearList = { 받은건수: all.items.length, 전체건수: all.total };
  ...
  return {
    ...
    ...(truncatedYearList
      ? {
          잘림: true,
          잘린조회: truncatedYearList,
          유의사항:
            `★ ${year}년 정보공개서가 ${truncatedYearList.전체건수}건인데 ` +
            `${truncatedYearList.받은건수}건만 받았습니다. 이름 필터는 받아온 범위 안에서만 ` +
            `돌았으므로, 찾는 가맹본부가 없다고 단정하지 마세요.`,
        }
      : { 잘림: false }),
