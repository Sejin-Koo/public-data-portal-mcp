...
//
//   ★★ 그런데 실제 배포(Vercel) 후 라이브 호출로는 매번 `fetch failed`로 실패한다
//   (2026-09-10 실측 — 이 컨테이너 샌드박스에서 직접 호출도 동일하게 503 "upstream
//   connect error"). example.com 등 다른 도메인은 같은 환경에서 정상 접속되므로
//   일반적인 아웃바운드 차단이 아니라 **SWIT 쪽이 클라우드/데이터센터 IP 대역을
//   막고 있을 가능성**이 높다. 즉 이 3단계는 코드·파싱 로직은 맞지만 **현재
//   프로덕션에서는 사실상 항상 폴백 실패로 끝난다** — 호출 자체는 안전하게
//   실패하므로(out.switError로 남고 예외를 던지지 않음) 다른 두 단계를 해치지는
//   않지만, "SWIT로 찾아줄 것"을 기대하고 이 경로를 쓰지는 말 것. 원인이 IP 차단이
//   맞는지, 다른 네트워크(예: 사용자 브라우저)에서는 되는지는 아직 미확인이다.
//   SW사업자 신고가 없는 회사라 못 찾는 것과, 네트워크가 막혀 못 찾는 것은 다르다 —
//   switError 필드로 구분해 알린다.
//
