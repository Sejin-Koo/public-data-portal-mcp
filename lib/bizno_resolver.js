...
//
// ★ 두 인덱스의 갱신 방식이 다르다. DART 인덱스는 매주 **통째로 교체**되고, 키스콘 색인은
//   월 1회 **누적 병합**된다(사명 변경 이력이 배열로 쌓이므로 옛 사명으로도 찾힌다).
//   그래서 두 파일을 하나로 합치면 안 된다 — 합치면 키스콘 누적분이 다음 주에 날아간다.
//
// ★ 휴업·폐업 상태는 어느 색인에도 없다. 상태가 필요하면 국세청 사업자등록 상태조회를
...
// ── 키스콘 누적 색인 ─────────────────────────────────────────────────────────
//
// 파일에는 사업자등록번호 → {n:[상호 최신순], r:대표자, a:지역, i:업종, d:최근관측일}만
// 담는다. 상호 → 번호 역방향은 파일 크기를 두 배로 만들므로 저장하지 않고 여기서 만든다.
 ...
 *
 * ★ 두 색인 어디에도 없는 회사(DART 미등록 + 건설업 미등록)는 해석이 불가능하다.
 *   실패 사유를 reason에 담아 돌려주되, **"그 회사가 존재하지 않는다"는 뜻이 아니다.**
...
export async function resolveBizNo(companyName) {
  ...
  if (kis.length > 1) {
    ...
    out.reason =
      ...
      (nowNamed < kis.length
        ...
        : "") +
      ". candidates에서 대표자·지역·업종·최근관측일로 고른 뒤 사업자등록번호를 직접 넘기세요.";
    return out;
  }
  ...
  if (!hit) {
    ...
    out.reason =
      "DART 고유번호 인덱스와 키스콘 건설업체 색인 어디에서도 회사를 찾지 못했습니다. " +
      "DART 미등록 비상장사이면서 건설업 등록도 없는 법인이거나, 사명이 변경됐거나, " +
      "인덱스 스냅샷 이후 신규 설립·등록된 법인일 수 있습니다. " +
      "**그 회사가 존재하지 않는다는 뜻은 아닙니다.**";
  }
  return out;
}
