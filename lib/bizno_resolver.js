...
//   실측(2026-09-10): "포니링크" 검색 → 1198137606, DART 기업개황의 실제 사업자등록번호와
//   정확히 일치해 신뢰성을 확인했다. 이름 검색만 지원해 정방향(회사명→번호)에서만 쓴다.
//   SW사업자 신고가 없는 회사는 이 경로로도 못 찾지만, 그 자체가 정상이다 — 신고 대상이
//   아닌 업종일 뿐 "회사가 없다"는 뜻이 아니다. 정적 인덱스가 아니라 매 호출마다 실시간
//   조회하므로(외부 왕복 있음) 세 번째 순서로 둔다.
//
