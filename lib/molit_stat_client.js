...
// ★ 확장자를 봐야 한다. 파일명에 "2009년12월"이 들어간 .hwp 첨부가 있어서
//   기준월만 보고 '월간'으로 분류하면 월간 xlsx 목록이 오염된다.
...
export async function getMolitVehicleStat(args = {}) {
  ...
  if (mode === "list") {
    ...
    const seen = new Map();
    for (const e of list) {
      if (!e.기준월 || e.유형 !== "월간") continue; // .hwp 등은 월간 목록에서 제외
    ...
    // 월간도 연도묶음도 아닌 첨부(.hwp 등). 월간 목록에 섞이면 안 되지만
    // 존재 자체는 알려야 "그 연도 자료가 아예 없다"는 오판을 막는다.
    const others = [];
    const seenO = new Set();
    for (const e of list) {
      if (e.유형 !== "기타" || !e.표시파일명) continue;
      if (seenO.has(e.표시파일명)) continue;
      seenO.add(e.표시파일명);
      others.push({ 수록연도: e.수록연도, 파일명: e.표시파일명, 확장자: e.확장자 });
    }
    ...
    return {
      ...
      기타첨부: {
        안내: "월간 xlsx도 연도묶음 zip도 아닌 첨부입니다(.hwp 등). 이 도구로는 열지 못합니다.",
        건수: others.length,
        목록: others.slice(0, 20),
      },
      중복게시월수: dup,
      ...(dup
