...
export async function searchDrugPermission({
  ...
  // ★ bizrno는 이 오퍼레이션(getDrugPrdtPrmsnDtlInq06)의 공식 파라미터 목록에 없지만 실제로
  //   동작한다(실측 2026-08-26: item_name=우루사 단독 10건 → bizrno=1248601143 추가 시 9건,
  //   대웅바이오의 알파우루사연질캡슐이 빠짐). 미문서화 파라미터이므로 어느 날 필터가 통째로
  //   무시되고 건수가 원래대로 돌아오면 이 점을 먼저 의심할 것.
...
export async function searchHealthFood({
  ...
  // ★ 명칭 통일(2026-08-26). 업체명 부분검색 파라미터는 형제 도구들과 같은 `entpName`이다.
  //
  // `companyName`도 계속 받는다. 다만 건강기능식품 원 API(getHtfsList01)에는 사업자등록번호
  // 파라미터가 아예 없어서, 의약품 3개 도구처럼 법인 단위로 정확히 좁히는 것이 **불가능**하다.
  // 그래서 같은 의도("이 회사로 좁혀줘")로 받되 할 수 있는 최선인 업체명 부분검색으로
  // 처리하고, 그 사실을 resolution에 명시한다 — 같은 파라미터가 도구마다 조용히 다르게
  // 동작하는 상황을 만들지 않기 위해서다.
  const company = entpName || companyName;
  const resolution = company
    ? {
        입력회사명: company,
        resolvedVia: "업체명 부분검색(사업자등록번호 미지원)",
        bizNo: null,
        note:
          "건강기능식품 원 API에는 사업자등록번호 파라미터가 없어 법인 단위 정확검색이 " +
          "불가능합니다. 의약품 3개 도구(search_pill_identification 등)의 companyName과 달리 " +
          "여기서는 업체명 부분검색으로만 좁혀지므로 계열사·동명 업체가 섞일 수 있습니다 — " +
          "결과의 ENTRPS를 확인하세요.",
      }
    : null;
  ...
  const list = await callPaged({
    ...
    params: { type: "json", Entrps: company, Prduct: productName, Sttemnt_no: statementNo },
    want: limit,
    perPage: Math.min(100, Math.max(limit, 10)),
    maxPages: 3,
  });
