...
export async function searchCalsQualityTests({
  ...
  if (contractorName) {
    ...
    for (const x of got.items) {
      const k = `${trim(x.cwkNm)}|${trim(x.cstrNm)}`;
    ...
    const 현장 = [...sites.values()].sort((a, b) =>
      String(b.준공예정일 || "").localeCompare(String(a.준공예정일 || ""))
    );
  ...
  const 현장 = [...sites.values()].sort((a, b) => b.검사건수 - a.검사건수);
