...
async function npsLookup(companyName, { months = 3 } = {}) {
  ...
  const list = await callApi({
    ...
    // ★ 60건 고정이다. 이름이 흔한 회사는 일치하는 사업장이 60건 밖에 남아 조용히 누락될 수
    //   있으므로, totalCount가 이보다 크면 아래에서 잘림을 알린다(2026-09-02 추가).
  ...
  const 검색잘림 =
    list.totalCount > list.items.length
      ? `★ 검색어 "${query}"에 ${list.totalCount}건이 걸렸는데 ${list.items.length}건만 받았습니다. ` +
        `일치하는 사업장이 받지 못한 범위에 있을 수 있으니, 결과가 비거나 적으면 회사명을 더 ` +
        `구체적으로 주고 다시 조회하세요.`
      : null;
