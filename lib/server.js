...
export function buildServer() {
  ...
  server.tool(
    ...
    "식약처 **의약품 제품 허가정보** 전체 DB를 조회합니다. 허가일자, 전문/일반 구분, 성상, 포장단위, " +
      ...
      "'콘드로이친'같이 짧은 핵심어로 넣어야 걸립니다. verbose=true로 부르면 효능효과·용법용량·" +
  ...
  server.tool(
    ...
    "식약처 **의약품 낱알식별 정보**를 조회합니다. 알약의 각인(PRINT_FRONT/BACK), 모양(DRUG_SHAPE), " +
      ...
      "전문/일반 구분이 나옵니다. ★ 검색 조건으로 실제 동작하는 것은 제품명·업체명·품목기준코드·" +
      "보험코드·성상(chart)·제형(formCodeName) 뿐이고, **각인·모양·색상은 요청 파라미터로 넣어도 " +
      "무시**됩니다(실측 확인). 따라서 '흰색 원형에 T가 찍힌 알약이 뭐야' 같은 겉모양 역추적은 이 " +
      "도구만으로는 불가능하며, 제품명 후보를 먼저 좁힌 뒤 반환된 값과 대조하는 방식으로 써야 합니다.",
    {
      itemName: z.string().optional().describe("제품명 부분검색"),
      entpName: z.string().optional().describe("업체명 부분검색"),
      itemSeq: z.string().optional().describe("품목기준코드"),
  ...
  server.tool(
    ...
    "식약처 **건강기능식품 품목정보**를 조회합니다. 업체명(ENTRPS), 제품명(PRDUCT), 신고번호" +
      ...
      "확인할 때 필수입니다. ★ 검색 조건은 제품명(productName) 부분일치와 신고번호뿐이고 업체명 " +
      "검색 파라미터는 확인되지 않았으므로, 업체로 좁히려면 제품명으로 받은 뒤 ENTRPS로 필터링하세요.",
    {
