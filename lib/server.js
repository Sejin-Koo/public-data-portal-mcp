...
import {
  ...
  VEHICLE_CODE_TABLES,
  DATA_START_YM,
} from "./vehicle_regist_client.js";
import {
  getVehicleRegistrationStock,
  getKosisVehicleCodeTables,
  KOSIS_VEHICLE_CODES,
} from "./kosis_vehicle_client.js";
import { getMolitVehicleStat } from "./molit_stat_client.js";
import { resolveCompanyIdentifiers } from "./bizno_resolver.js";
...
export function buildServer() {
  ...
  server.tool(
    "resolve_bizno",
    "회사명 ↔ **사업자등록번호**를 양방향으로 해석해 기업 식별자를 한 묶음으로 돌려줍니다. " +
      "사업자등록번호·법인등록번호·DART 고유번호·종목코드·상호(사명 변경 이력 포함)·대표자·소재지. " +
      "**기업 조사에서 사업자등록번호가 필요할 때는 다른 도구로 헤매지 말고 이 도구를 먼저 부르세요.** " +
      "공공데이터포털 계열 도구(고용·산재보험, 식약처, 나라장터 낙찰, 가맹본부, 키스콘)는 회사를 " +
      "사업자등록번호 완전일치로만 좁히는 경우가 많아, 회사명 부분검색으로는 그룹 접두어에서 계열사가 " +
      "딸려 들어옵니다. " +
      "★ **해석 경로는 3단계입니다.** ①DART 고유번호 인덱스 → 기업개황(상장·외감법인) " +
      "②키스콘 건설업체 누적 색인(DART 미등록 소규모 비상장사까지 — 단 건설업 등록업체에 한함) " +
      "③SWIT 소프트웨어사업자 신고 검색(DART 미등록 IT·SI 비상장사 — 실시간 조회). " +
      "한 곳으로 풀리면 어느 경로였는지가 해석경로에 적힙니다. **후보가 여럿이면 해석이 " +
      "완료되지 않아 해석경로가 null이고**, 출처는 각 후보의 출처 필드에 있습니다. " +
      "★ **①②는 이름을 정규화해 대조하지만 ③ SWIT는 이름을 포함하는 부분일치 검색입니다.** " +
      "그래서 3단계로 풀린 건은 후보가 훨씬 잘 늘어납니다(실측: '링크아이' → 3곳). " +
      "후보가 많다고 도구가 잘못 동작한 것이 아닙니다. " +
      "★ **못 찾았다고 '그런 회사가 없다'고 답하지 마세요.** DART에도 없고 건설업 등록·SW사업자 " +
      "신고도 없는 법인이면 세 소스 어디에도 안 나옵니다 — 회사가 존재하지 않는다는 뜻이 아닙니다. " +
      "★ **3단계는 '신고 없음'(정상)과 '조회 실패'가 다릅니다.** 후자는 SWIT조회오류 필드가 " +
      "오고 안내에도 '(SWIT 조회 자체는 실패했습니다: …)'가 붙습니다 — 그 둘 중 하나라도 " +
      "보이면 SW사업자 등록이 없다고 단정하지 말고 조회 실패 사실을 밝히세요. " +
      "**서버에 NIMBLE_API_KEY가 설정돼 있지 않으면 3단계는 항상 이 실패로 끝납니다**(SWIT가 " +
      "클라우드 IP를 차단해 국내 IP 경유 프록시가 필요하기 때문입니다). " +
      "★ **국세청 등록 상태를 함께 돌려줍니다(국세청상태).** 계속사업자·휴업자·폐업자와 폐업일· " +
      "과세유형이고, 색인이 아니라 **조회 시점에 국세청에 실시간으로 물은 값**입니다. " +
      "폐업·휴업이면 상태경고가 함께 오니 재무·수주 이력을 이어서 조회하기 전에 먼저 밝히세요. " +
      "후보가 여럿일 때도 각 후보에 상태가 붙고 후보안내로 살아 있는 곳이 몇 곳인지 알려줍니다 — " +
      "폐업한 후보를 먼저 제외하고 고르세요. 조회가 실패하면 국세청상태조회오류가 대신 오는데 " +
      "**그것을 폐업으로 읽지 마세요.** 국세청에 등록 이력이 없는 번호는 '국세청에 등록되지 않은 " +
      "번호'로 표기되는데 이것도 폐업과 다릅니다. " +
      "★ **`건설업면허업종`은 키스콘의 건설업 등록 면허 업종이지 그 회사의 주력 업종이 아닙니다.** " +
      "삼성전자에도 건설업 면허가 있어 '금속구조물·창호·온실공사업'이 붙습니다 — 업종 판정 근거로 쓰지 마세요. " +
      "★ 같은 상호의 업체가 여럿이면 하나를 고르지 않고 **후보**에 전부 담아 돌려줍니다. " +
      "대표자·지역·건설업면허업종·국세청상태로 고른 뒤 그 사업자등록번호를 다음 도구에 넘기세요. " +
      "★ 색인은 스냅샷입니다(DART 주 1회 교체 / 키스콘 월 1회 누적 병합). 응답의 색인기준 날짜 이후에 " +
      "설립·등록·개명된 법인은 아직 없을 수 있습니다. 3단계 SWIT와 국세청 상태는 실시간이라 " +
      "색인기준에 날짜가 없고, 국세청 조회 시점은 **국세청상태조회시각** 필드에 별도로 옵니다 — " +
      "근거 시점을 밝힐 때 그 값을 쓰세요.",
    {
      companyName: z
        .string()
        .optional()
        .describe("회사명. (주)·주식회사·공백 표기 차이는 무시합니다. 옛 사명으로도 찾힙니다"),
      bizNo: z
        .string()
        .optional()
        .describe("사업자등록번호 10자리(하이픈 무관). 주면 번호 → 상호 역방향으로 해석합니다"),
    },
    async (args) => {
      const result = await resolveCompanyIdentifiers(args);
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    }
  );
  ...
  server.tool(
    "get_new_vehicle_registrations",
    "한국교통안전공단 자동차종합정보로 **신규등록(신차) 대수**를 월별로 조회합니다(수록 " +
      `${DATA_START_YM}~현재, 당월분은 익월 2일 반영). 차종·등록지역·사용연료·용도·성별·연령대·` +
      "배기량·국산수입 중 한 축의 분포를 받거나, 나머지 축을 고정조건으로 걸 수 있습니다. " +
      "★ **누적 등록대수(스톡)가 아닙니다.** 이 도구가 주는 값은 그 달에 새로 등록된 대수(플로우)이고, " +
      "'전기차가 지금까지 몇 대 등록돼 있나' 같은 누적 질문에는 쓸 수 없습니다 — 월별 신규등록을 " +
      "합산해도 말소(폐차·수출)를 빼지 못해 누적 등록대수가 되지 않습니다. 누적은 국토교통부 " +
      "자동차등록현황보고(KOSIS 116/DT_MLTM_5498 등)가 담당합니다. " +
      "★ 이 API는 조건 하나당 **건수 하나**만 돌려주므로 분포는 서버가 코드값 수만큼 호출해 만듭니다. " +
      "필요 호출 수는 개월 수 × (분포 축 값 수 + 1)이고 상한을 넘으면 조회하지 않고 거절합니다 " +
      "(일일 트래픽 3,000회, 계정 단위 공유). 응답의 upstream호출수로 소모량을 확인하세요. " +
      "★ 분포를 받으면 **합계검증** 필드에 '분포 합 = 월 합계'가 맞는지 실어 보냅니다. " +
      "'불일치'가 나오면 그 축은 전수 분할이 아니므로 구성비 계산에 그대로 쓰지 마세요.",
    {
      from: z
        .string()
        .describe(`조회 시작 등록월 YYYYMM (예: 202501). ${DATA_START_YM} 이전은 자료가 없습니다`),
      to: z.string().optional().describe("조회 종료 등록월 YYYYMM. 생략 시 from 한 달만 조회"),
      breakdown: z
        .enum(BREAKDOWN_KEYS)
        .optional()
        .describe(
          "분포를 받을 축 하나. none(기본)=월별 합계만 / vehicleType=차종(4) / region=등록지역(17) / " +
            "fuel=사용연료(11) / usage=용도(3) / gender=성별(3) / ageGroup=연령대(10) / " +
            "displacement=배기량(11) / origin=국산수입(2). 괄호 안은 값 개수이며 그만큼 호출이 늘어납니다"
        ),
      filters: z
        .object({
          vehicleType: z.string().optional().describe("차종코드 1승용 2승합 3화물 4특수"),
          region: z.string().optional().describe("등록지역코드 1서울~17제주"),
          fuel: z
            .string()
            .optional()
            .describe("사용연료코드 1CNG 2경유 3수소 4엘피지 5전기 6하이브리드(CNG+전기) 7하이브리드(휘발유+전기) 8휘발유 9휘발유(무연) 10휘발유(유연) 11기타"),
          usage: z.string().optional().describe("용도 1자가용 2영업용 3관용"),
          gender: z.string().optional().describe("성별: 남자 / 여자 / 법인"),
          ageGroup: z.string().optional().describe("연령대 0법인 1(10대)~9(90대)"),
          displacement: z.string().optional().describe("배기량코드 1(800cc미만)~11(5000cc이상)"),
          origin: z.string().optional().describe("국산수입: 국산 / 외산"),
          carNameCode: z.string().optional().describe("차명코드 6자리(예: 000004)"),
          modelYear: z.string().optional().describe("모델년도 YYYY"),
        })
        .optional()
        .describe("모든 호출에 함께 걸 고정조건. breakdown으로 지정한 축은 여기 넣을 수 없습니다"),
      showCodeTables: z
        .union([z.boolean(), z.enum(["true", "false"])])
        .transform((v) => v === true || v === "true")
        .optional()
        .describe("true면 조회 없이 코드표만 돌려줍니다"),
    },
    async ({ from, to, breakdown, filters, showCodeTables }) => {
      if (showCodeTables) {
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify(
                { 코드표: VEHICLE_CODE_TABLES, 수록시작: DATA_START_YM },
                null,
                2
              ),
            },
      ...
      try {
        const result = await getNewVehicleRegistrations({ from, to, breakdown, filters });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (e) {
        return { content: [{ type: "text", text: JSON.stringify({ 오류: e.message }, null, 2) }] };
      }
    }
  );

  server.tool(
    "get_vehicle_registration_stock",
    "KOSIS 국토교통부 자동차등록대수현황으로 **누적 등록대수(스톡)** 를 월별·시도별로 조회합니다" +
      "(2011.01~현재, 월말 기준). 차종(승용·승합·화물·특수·총계)과 용도(계·자가용·영업용·관용) " +
      "축이 있고, 시도 하나를 지정하면 시군구까지 내려갑니다. " +
      "★ **그 달의 신규등록이 아닙니다.** 신차 등록은 get_new_vehicle_registrations가 담당하고, " +
      "둘은 스톡과 플로우로 성격이 다릅니다 — 신규등록을 합산해도 말소(폐차·수출)가 빠지지 않아 " +
      "누적이 되지 않습니다. " +
      "★★ **이 통계표에는 연료 축이 없습니다.** '전기차가 지금까지 몇 대 등록돼 있나'는 이 도구로 " +
      "답할 수 없습니다 — 연료별 누적은 KOSIS·국토교통 통계누리 어느 통계표에도 없고, 국토교통부 " +
      "보도자료(1·7월 발표, 반기)와 e-나라지표 1257(연 1회)에만 실립니다. 그 경우 이 도구를 " +
      "돌리지 말고 그 출처를 확인하세요. " +
      "★ 광주·전남은 **2026년 7월부터 전남광주로 통합**되었습니다(2026-06까지 17개 시도, " +
      "2026-07부터 16개). 통합 이후 월을 '광주'·'전남'으로, 통합 이전 월을 '전남광주'로 " +
      "조회하면 0건이 나오는데 둘 다 자료 누락이 아닙니다. " +
      "★ 전국 합계 행은 원자료에 없어 시도별 '총계 × 계'를 합산해 만든 값으로 함께 돌려줍니다.",
    {
      from: z.string().describe("조회 시작 기준월 YYYYMM (예: 202601). 2011년 1월부터 수록"),
      to: z.string().optional().describe("조회 종료 기준월 YYYYMM. 생략 시 from 한 달만"),
      level: z
        .enum(["sido", "sigungu"])
        .optional()
        .describe(
          "sido(기본)=시도 단위 / sigungu=시군구 단위. sigungu는 region을 반드시 지정해야 합니다"
        ),
      region: z
        .enum(KOSIS_VEHICLE_CODES.시도)
        .optional()
        .describe(`시도명. ${KOSIS_VEHICLE_CODES.시도.join(", ")}`),
      vehicleType: z
        .enum(KOSIS_VEHICLE_CODES.차종)
        .optional()
        .describe("차종. 생략 시 전체"),
      usage: z
        .enum(KOSIS_VEHICLE_CODES.항목)
        .optional()
        .describe("용도. 생략 시 전체"),
      showCodeTables: z
        .union([z.boolean(), z.enum(["true", "false"])])
    ...
    async ({ from, to, level, region, vehicleType, usage, showCodeTables }) => {
      if (showCodeTables) {
        return {
          content: [
            { type: "text", text: JSON.stringify(getKosisVehicleCodeTables(), null, 2) },
          ],
        };
      }
      try {
        const result = await getVehicleRegistrationStock({
          from,
          to,
          level,
          region,
  ...
  server.tool(
    "get_molit_vehicle_stat",
    "국토교통 통계누리가 매월 게시하는 **자동차 등록자료 통계 엑셀**을 서버가 직접 받아 읽습니다. " +
      "★ 이 파일에만 있는 축: **연료별 누적 등록대수**(전기·수소·하이브리드를 결합연료별까지 구분), " +
      "**말소등록(자진·직권 사유별)**, 성별·연령별, 차령별, 최대적재량별, 배기량별, 승차정원별, " +
      "초소형 규모별, 수입차 시군구별. `get_vehicle_registration_stock`(KOSIS)에는 연료 축이 없으므로 " +
      "'전기차가 지금까지 몇 대 등록돼 있나' 같은 질문은 이 도구로 답합니다. " +
      "★ KOSIS보다 공표가 빠릅니다 — KOSIS에서 0건으로 나오는 달도 여기엔 이미 올라와 있을 수 있습니다. " +
      "★ 한 파일 안에서 시도 축이 갈립니다: 누적 시트는 전남광주 통합 축, 신규등록 시트는 광주·전남 분리 축입니다. " +
      "쓰는 순서는 mode='list'(게시된 월 확인) → 'sheets'(시트 이름 확인) → 'data'(원하는 시트 읽기)입니다.",
    {
      mode: z
        .enum(["list", "sheets", "data", "diagnose"])
        .optional()
        .describe(
          "list(기본)=게시된 월 목록 / sheets=그 달 파일의 시트 이름 / data=시트 한 장을 행 배열로 / " +
            "diagnose=접속과 앵커 파싱 상태 점검(파라미터 매핑이 깨졌을 때만)"
        ),
      yearMonth: z.string().optional().describe("기준월 YYYYMM (mode=sheets/data 필수)"),
      kind: z
        .enum(["자동차", "이륜자동차"])
        .optional()
        .describe("같은 목록에 이륜자동차 신고현황이 함께 게시됩니다. 기본은 자동차"),
      sheet: z
        .string()
        .optional()
        .describe(
          "읽을 시트. 전체 이름('10.연료별_등록현황'), 부분 이름('연료별'), 번호('10') 모두 됩니다"
        ),
      maxRows: z.number().optional().describe("반환 행 상한(기본 400, 최대 2000)"),
      limit: z.number().optional().describe("mode=list 에서 돌려줄 월 수(기본 40)"),
      downloadQuery: z
        .string()
        .optional()
        .describe(
          "다운로드 쿼리스트링 직접 지정(예: 'atchFileId=FILE_000...&fileSn=0'). " +
            "사이트가 앵커 구조를 바꿔 자동 파싱이 깨졌을 때만 씁니다"
        ),
    },
    async (args) => {
      try {
        const result = await getMolitVehicleStat(args);
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (e) {
        return { content: [{ type: "text", text: JSON.stringify({ 오류: e.message }, null, 2) }] };
      }
