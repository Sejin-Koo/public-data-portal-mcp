...
export function buildServer() {
  ...
  server.tool(
      ...
      "행정처분·폐업 이력은 search_construction_sanctions를 쓰세요.",
    {
      ...
      maxPages: z.number().int().min(1).max(10).optional().describe("오퍼레이션당 최대 페이지(5,000행 단위, 기본 3)"),
    },
    async (args) => {
  ...
  server.tool(
    ...
    "국토교통부 키스콘(KISCON) **건설업체 행정처분·가처분·폐업 공시**를 조회합니다. 협력사·하도급사· " +
      ...
      "실사 목적이라면 기간을 수년 단위로 넓혀 재조회해야 합니다. 답변에는 확인한 기간을 반드시 " +
      "밝히세요. 가처분(ncrPdStatus=Y)이 걸린 건은 처분 효력이 정지 중일 수 있습니다.",
    {
  ...
  server.tool(
    ...
    "국토교통부 키스콘(KISCON) **건설공사대장 통보 통계**를 조회합니다. 일자 × 지역 × 발주구분" +
      ...
      "★ **금액의 단위가 원 명세에 없습니다.** 건당 평균으로 미루어 억원으로 보이나 확인된 사실이 " +
      "아니므로, 절대 금액을 인용할 때는 단위 미확인임을 함께 밝히고 추세·비중 비교 위주로 쓰세요.",
    {
      sDate: z.string().describe("조회 시작일 YYYYMMDD (필수)"),
