...
export function buildServer() {
  ...
  server.tool(
    ...
    "국토교통부 키스콘(KISCON) **건설업체 등록·변동 공시**를 조회합니다. 건설업 신규등록·등록기준사항 " +
      ...
      "특정 회사를 찾는 것이 목적이면 기간을 넓혀 재조회하되 **기간을 여러 구간으로 쪼개지 마세요** — " +
      "구간 경계에서 같은 공시가 중복 집계되어 합계가 부풀려집니다. 한 번에 길게 조회하는 편이 정확하며, " +
      "업체 필터를 주면 마지막 페이지까지 자동 수집합니다. 답변에는 확인한 기간을 반드시 밝히세요. " +
      ...
      "행정처분·폐업 이력은 search_construction_sanctions를 쓰세요.",
    {
      ...
      maxPages: z.number().int().min(1).max(100).optional().describe(
        "오퍼레이션당 최대 페이지(5,000행 단위, 기본 20). companyName·bizNo를 주면 누락을 막기 위해 " +
          "이 값과 무관하게 마지막 페이지까지 자동 수집합니다"
      ),
    },
    async (args) => {
  ...
  server.tool(
    ...
    "국토교통부 키스콘(KISCON) **건설업체 행정처분·가처분·폐업 공시**를 조회합니다. 협력사·하도급사· " +
      ...
      "실사 목적이라면 기간을 수년 단위로 넓혀 재조회해야 합니다. 이때 **기간을 여러 구간으로 쪼개지 " +
      "마세요** — 구간 경계에서 같은 공시가 중복 집계되어 합계가 부풀려집니다. 한 번에 길게 조회하면 " +
      "되고, 업체 필터를 주면 마지막 페이지까지 자동 수집합니다. " +
