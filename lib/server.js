...
export function buildServer() {
  ...
  server.tool(
    ...
    "회사명 ↔ **사업자등록번호**를 양방향으로 해석해 기업 식별자를 한 묶음으로 돌려줍니다. " +
      ...
      "딸려 들어옵니다. " +
      "★ **해석 경로는 2단계입니다.** ①DART 고유번호 인덱스 → 기업개황(상장·외감법인) " +
      "②키스콘 건설업체 누적 색인(DART 미등록 소규모 비상장사까지 — 단 건설업 등록업체에 한함). " +
      "어느 경로로 풀렸는지는 응답의 해석경로에 그대로 적힙니다. " +
      "★ **못 찾았다고 '그런 회사가 없다'고 답하지 마세요.** DART에도 없고 건설업 등록도 없는 " +
      "법인이면 이 두 색인 어디에도 안 나옵니다 — 회사가 존재하지 않는다는 뜻이 아닙니다. " +
      "★ **휴업·폐업 상태는 이 도구가 답하지 않습니다.** 색인에 상태를 굳혀 두면 낡은 값을 답하게 되므로 " +
      "일부러 뺐습니다. 현재 상태가 필요하면 국세청 사업자등록 상태조회를 따로 확인하세요. " +
      ...
      "대표자·지역·건설업면허업종으로 고른 뒤 그 사업자등록번호를 다음 도구에 넘기세요. " +
      "★ 색인은 스냅샷입니다(DART 주 1회 교체 / 키스콘 월 1회 누적 병합). 응답의 색인기준 날짜 이후에 " +
      "설립·등록·개명된 법인은 아직 없을 수 있습니다.",
    {
      companyName: z
        .string()
        .optional()
