...
export function buildServer() {
  ...
  server.tool(
    ...
    "조달청 나라장터 **낙찰정보**를 조회해 공공입찰의 **최종낙찰업체·최종낙찰금액·최종낙찰률**을 " +
      ...
      "★ 조회기간 상한은 약 " + RANGE_LIMIT_DAYS + "일입니다(초과 시 원 API가 resultCode 07 '입력범위값 초과'). " +
      "더 긴 기간을 주면 서버가 " + RANGE_LIMIT_DAYS + "일 창으로 잘라 순차 호출하고, 호출 예산을 넘기면 잘림=true로 " +
      "표시합니다. **잘림=true인 결과로 낙찰가율 평균 같은 통계를 내지 마세요.** " +
      ...
      "전 참가업체의 순위별 투찰금액이 필요하면 get_narajangteo_openg_result를 쓰세요.",
    {
      ...
      indstrytyNm: z.string().optional().describe("업종명 부분검색"),
      ...
      bizno: z.string().optional().describe("업체 사업자등록번호 10자리. 그 업체의 낙찰건만"),
      limit: z.number().int().min(1).max(300).optional().describe("최대 반환 건수 (기본 50)"),
    },
    async (args) => {
