...
export async function searchConstructionFirms({
  ...
  return {
    ...
    note:
      ...
      "★ 잘림=true면 조회기간에 공시가 더 있는데 시간 제한으로 다 받지 못한 것입니다. 이때는 기간을 연 단위로 나눠 여러 번 호출하고, **건수를 더하지 말고 공고번호·업체명이 같은 행을 ncrGsSeq 기준으로 합집합** 하세요 — 구간 경계에 걸친 공시가 양쪽에 모두 들어와 단순 합산하면 부풀려집니다(실측: 23년치를 8분할하면 53,229행, 단일 조회는 53,054행). " +
