...
// ★ 이 API는 한 요청당 20~40초가 걸린다(행 수와 거의 무관한 서버측 조회시간).
// 동시 요청은 실제로 병렬 처리되지만 6개를 넘기면 서버가 오류 페이지를 돌려주기 시작한다.
// 실측(2003~2026 행정처분 53,054행): 5,000행 페이지 1건 27초, 10,000행 6페이지 동시 84초.
// 즉 20년치 전량은 MCP 60초 제한 안에 들어오지 않는다 — 아래 truncated 안내 참조.
const PAGE_CONCURRENCY = 6;

/**
 * @param keep 행 필터. 주면 받자마자 걸러 메모리에 안 쌓는다(20년치 5만행 대응).
 */
async function fetchAllPages(baseUrl, op, params, maxPages = 20, keep = null) {
  const take = (arr) => (keep ? arr.filter(keep) : arr);

  const first = await callKiscon(baseUrl, op, { ...params, pageNo: 1, numOfRows: MAX_ROWS });
  let items = take(first.items);
  ...
  for (let i = 0; i < rest.length; i += PAGE_CONCURRENCY) {
    ...
    const got = await Promise.all(
      batch.map((p) => callKiscon(baseUrl, op, { ...params, pageNo: p, numOfRows: MAX_ROWS }))
    );
    for (const g of got) {
      fetched += g.items.length;
      items = items.concat(take(g.items));
    }
  }
  ...
  return {
    ...
    truncated: first.totalCount > fetched,
  };
}
...
async function gatherFirmOps({ kinds, sDate, eDate, area, areaDetail, target, companyName, itemName, maxPages }) {
  // ★ 끝까지 페이징하면 20년치 조회가 60초 제한에 걸려 아예 응답을 못 준다.
  // 받을 수 있는 만큼 받고, 남으면 잘림=true로 알려 호출자가 기간을 나눠 재조회하게 한다.
  const pageCap = maxPages;
  const nameKey = !target && companyName ? normalizeCorpKey(companyName) : null;
  const hasFilter = Boolean(target || nameKey || itemName);
  // 받는 즉시 걸러 20년치 5만행을 메모리에 쌓지 않는다.
  const keep = hasFilter
    ? (x) => {
  ...
  const rows = [];
  const perOp = [];
  // 오퍼레이션끼리도 서로 독립이므로 동시에 돌린다(장기 조회 응답시간 확보).
  const metas = kinds.map((k) => FIRM_OPS[k]).filter(Boolean);
  const results = await Promise.all(
    metas.map(async (meta) => {
      try {
        ...
        return { meta, error: String(e.message || e) };
      }
    })
  );
  for (const { meta, r, error } of results) {
    if (error) {
      perOp.push({ 구분: meta.label, 오퍼레이션: meta.op, error });
      continue;
    }
    perOp.push({
      구분: meta.label,
      ...
      수집행: r.fetched,
      조건일치_행: r.items.length,
      조건일치_건: uniqueCount(r.items),
      잘림: r.truncated,
    });
    for (const x of r.items) rows.push({ __kind: meta.label, ...x });
  }

  // keep에서 이미 걸렀으므로 rows가 곧 조건일치 행이다.
  const fetchedRows = perOp.reduce((a, o) => a + (o.수집행 || 0), 0);
  return { perOp, rows, filtered: rows, fetchedRows };
}
...
export async function searchConstructionFirms({
  ...
  const { perOp, filtered, fetchedRows } = await gatherFirmOps({
  ...
  return {
    ...
    note:
      ...
      "★★ **이 응답의 집계값은 다른 호출의 집계값과 더하면 안 됩니다.** 기간을 나눠 여러 번 호출하는 것 자체는 괜찮지만, 구간 경계에 걸친 공시가 양쪽에 모두 들어오므로 구간별 건수·금액을 합산하면 부풀려집니다 — 실측으로 2년치를 8분할해 합산했더니 건수는 +0.42%, **과징금 총액은 +3.22%(6.7억원)** 과대 집계됐습니다. 부득이 합산했다면 답변에 '분할 조회 합산이라 경계 중복이 포함된 근사치'임을 반드시 밝히고, 가능하면 조회기간을 좁혀 한 번에 받으세요(안전 한계는 약 25,000행). 잘림=true면 시간 제한으로 다 받지 못한 것이니 같은 원칙이 적용됩니다. " +
      "공시일(ncrGsDate)에는 미래·과거 오류값이 섞여 있어 등록일(ncrGsRegdate)을 씁니다.",
  };
}
...
export async function searchConstructionSanctions({
  ...
  // ★ 상위 N개만 돌려주면 호출자가 "상위 N개 합산"으로 전체를 근사하게 되고, 그 값이 실제와
  // 어긋난다(실측: 2년치 위반내용은 85종). 종류가 많아도 전량 반환한다.
  const violationEntries = Object.entries(byViolation).sort((a, b) => b[1] - a[1]);
  const VIOLATION_CAP = 500;
  const violations = Object.fromEntries(violationEntries.slice(0, VIOLATION_CAP));
  ...
  return {
    ...
    처분유형별: byType,
    위반내용별: violations,
    위반내용_종류수: violationEntries.length,
    위반내용_절단: violationEntries.length > VIOLATION_CAP,
    금액합계_원: money,
    반환: items.length,
    잘림: perOp.some((o) => o.잘림),
    resolution,
    ...
    note:
      ...
      "★ 같은 처분 1건이 보유 업종 수만큼 행으로 반복됩니다. 처분유형별·위반내용별·금액합계는 이미 " +
