...
async function callHub(op, params, { timeoutMs = 30000, retries = 5 } = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      // ★ 페이지를 연달아 많이 요청하면 HTTP 200에 **빈 본문**이 오는 일이 있다(실측:
      //   서초동 74페이지를 연속 호출하던 중 발생, 개별 페이지를 따로 치면 정상).
      //   레이트리밋 성격이므로 오류로 확정하지 말고 더 길게 쉬었다가 재시도한다.
      if (!text.trim()) {
...
async function fetchHubPages(op, params, maxPages = 20) {
  ...
  for (let p = 2; p <= pages; p++) {
    // 대량 페이징 시 레이트리밋을 피하려고 페이지 사이에 간격을 둔다.
    ...
    try {
      ...
      // ★ 실패한 페이지를 조용히 넘기면 표본이 소리 없이 빠진다. 반드시 드러낸다.
...
export async function searchArchPermits({
  ...
  return {
    ...
    잘림: got.truncated,
    ...(got.실패페이지 ? { 실패페이지: got.실패페이지, 경고: "일부 페이지 수집에 실패했습니다. 이 결과로 통계(중앙값·사분위)를 내지 마세요." } : {}),
    필터후건수: rows.length,
