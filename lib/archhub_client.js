...
async function fetchHubPages(op, params, maxPages = 3) {
  ...
  return { items, totalCount: first.totalCount, truncated: first.totalCount > items.length };
}
...
export async function resolveBjdong(regionName, { limit = 500 } = {}) {
  const got = await callRegion({ locatadd_nm: regionName, pageNo: 1, numOfRows: Math.min(limit, MAX_ROWS) });
