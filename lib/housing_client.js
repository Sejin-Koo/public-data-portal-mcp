...
// ────────────────────────────────────────────────────────────────────────────
// 전국 전수 스냅샷 (data/housing_permits.json.gz)
// ────────────────────────────────────────────────────────────────────────────
//
// ★★ 이것이 API의 법정동 순회를 대체한다. 건축HUB "대용량 데이터 제공"에서 받은 전국
//    주택인허가 기본개요 + 관리공동형별개요를 지번으로 결합해 만든 스냅샷이다.
//    API는 법정동 단위라 전국 20,560개를 돌아야 하고 일일 트래픽이 10,000건이라
//    하루에 전국 스캔이 불가능한데, 이 파일은 한 번 읽으면 끝이다.
//
// ★ 대신 **기준월 시점의 스냅샷**이다. 그달 이후 신규 승인·착공은 없다.
//   최신 확인이 필요하면 search_housing_permits(API)로 그 지역만 다시 조회할 것.
//
// ★ 필터: 총세대수 30 이상 또는 주용도=공동주택. 단독주택·근린생활시설은 빠져 있다.
//
// ★ 사업주체명·난방방식·분양구분은 관리공동형별개요에서 오는데, 그 대장은 준공 단지
//   위주라 결합률이 41% 안팎이다(실측). 비어 있는 것이 정상이며 "그런 값이 없다"는
//   뜻이 아니다.

const BULK_PATH = join(__dirname, "..", "data", "housing_permits.json.gz");
let BULK = null;

function loadBulk() {
  if (BULK !== null) return BULK;
  try {
    const payload = JSON.parse(gunzipSync(readFileSync(BULK_PATH)).toString("utf-8"));
    const idx = {};
    payload.컬럼.forEach((c, i) => (idx[c] = i));
    BULK = { ...payload, idx };
  } catch (e) {
    BULK = { error: String(e.message || e) };
  }
  return BULK;
}

/** 전국 전수 스냅샷에서 단지를 찾는다. API 호출이 없다. */
export function scanHousingPermitsBulk({
  region,
  nameContains,
  developer,
  saleType,
  ...
  if (b.error) {
    return {
      ok: false,
      reason:
        `전수 스냅샷을 읽지 못했습니다: ${b.error}. data/housing_permits.json.gz가 저장소에 있는지 확인하세요. ` +
        "대신 search_housing_permits(API)로 지역을 지정해 조회할 수 있습니다.",
    };
  }
  const i = b.idx;
  const g = (r, c) => r[i[c]] || "";
  let rows = b.행;
  const 전체 = rows.length;

  // ★ 사용검사일이 비어 있다고 진행 중인 것이 아니다. 1990년대 준공 단지도 대장에
  //   사용검사일이 기재되지 않은 경우가 많다(실측: "음봉 초원아파트" 검사예정 1997-12-30이
  //   사용검사일 공란). 사용검사일 공란 **그리고** 사용검사예정일이 오늘 이후여야 진행 중이다.
  let 판정불가 = 0;
  if (onlyOngoing) {
    const today = new Date().toISOString().slice(0, 10).replace(/-/g, "");
    const before = rows.length;
    rows = rows.filter((r) => {
      if (g(r, "사용검사일")) return false;
      const sched = g(r, "사용검사예정일");
      if (!sched) return false; // 예정일조차 없으면 진행 여부를 알 수 없다 — 제외하고 세어 둔다
      return sched >= today;
    });
    판정불가 = before - rows.length;
  }
  if (region) {
    const q = norm(region);
    rows = rows.filter((r) => norm(g(r, "대지위치")).includes(q));
  }
  if (nameContains) {
    const q = norm(nameContains);
    rows = rows.filter((r) => norm(g(r, "건물명")).includes(q) || norm(g(r, "단지명")).includes(q));
  }
  if (developer) {
    const list = Array.isArray(developer) ? developer : [developer];
    rows = rows.filter((r) => list.some((d) => norm(g(r, "사업주체명")).includes(norm(d))));
  }
  if (saleType) {
    const list = Array.isArray(saleType) ? saleType : [saleType];
    rows = rows.filter((r) => list.some((t) => g(r, "분양구분").includes(t)));
  }
  if (heating) rows = rows.filter((r) => g(r, "난방방식").includes(heating));
  if (minHouseholds) rows = rows.filter((r) => Number(g(r, "총세대수") || 0) >= minHouseholds);
  if (maxHouseholds) rows = rows.filter((r) => Number(g(r, "총세대수") || 0) <= maxHouseholds);
  if (useInsptSchedFrom) rows = rows.filter((r) => g(r, "사용검사예정일") >= onlyDigits(useInsptSchedFrom));
  if (useInsptSchedTo) rows = rows.filter((r) => g(r, "사용검사예정일") && g(r, "사용검사예정일") <= onlyDigits(useInsptSchedTo));
  if (approvedFrom) rows = rows.filter((r) => g(r, "사업계획승인일") >= onlyDigits(approvedFrom));
  if (approvedTo) rows = rows.filter((r) => g(r, "사업계획승인일") && g(r, "사업계획승인일") <= onlyDigits(approvedTo));

  // ★ 같은 지번·건물명이 대장 건별로 반복되는 일이 있다(실측: 중외공원 1BL이 2행).
  //   그대로 세면 현장 수와 세대 합계가 부풀려진다. 지번+건물명으로 접고 세대수가 큰 행을 남긴다.
  const dedup = new Map();
  for (const r of rows) {
    const k = [g(r, "시군구코드"), g(r, "법정동코드"), g(r, "대지구분"), g(r, "번"), g(r, "지"), norm(g(r, "건물명"))].join("-");
    const prev = dedup.get(k);
    if (!prev || Number(g(r, "총세대수") || 0) > Number(g(prev, "총세대수") || 0)) dedup.set(k, r);
  }
  const 중복제거전 = rows.length;
  rows = Array.from(dedup.values());

  const 세대합 = rows.reduce((a, r) => a + (Number(g(r, "총세대수") || 0) || 0), 0);
  const 연도별 = {};
  const 분양구분별 = {};
  for (const r of rows) {
    const y = (g(r, "사용검사예정일") || "").slice(0, 4) || "미상";
    연도별[y] = (연도별[y] || 0) + 1;
    const t = g(r, "분양구분") || "(미기재)";
    분양구분별[t] = (분양구분별[t] || 0) + 1;
  }
  const sortIdx = i[sort] !== undefined ? i[sort] : i["사용검사예정일"];
  rows = rows.slice().sort((a, x) => String(a[sortIdx] || "99999999").localeCompare(String(x[sortIdx] || "99999999")));
  ...
  const toObj = (r) => {
    ...
    o.사업계획승인일 = ymdDash(o.사업계획승인일);
    o.착공예정일 = ymdDash(o.착공예정일);
    o.착공일 = ymdDash(o.착공일);
    o.사용검사예정일 = ymdDash(o.사용검사예정일);
    o.사용검사일 = ymdDash(o.사용검사일);
    o.진행상태 = o.사용검사일 ? "준공" : o.착공일 ? "시공중" : o.착공예정일 ? "착공예정" : "승인";
    return o;
  };
  ...
  return {
    ...
    전체_단지수: 전체,
    중복제거전_행수: 중복제거전,
    조건일치_단지수: rows.length,
    ...(onlyOngoing ? { 진행여부_판정불가_제외: 판정불가 } : {}),
    총세대_합계: 세대합,
    "사용검사예정_연도별": Object.fromEntries(Object.entries(연도별).sort()),
    분양구분별: 분양구분별,
    반환_단지수: Math.min(rows.length, limit),
    단지: rows.slice(0, limit).map(toObj),
    caveats: [
      `이 결과는 ${b.기준월} 기준 스냅샷입니다. 그 이후 신규 사업계획승인·착공은 반영돼 있지 않으므로, 최신이 필요하면 search_housing_permits로 해당 지역을 다시 조회하세요.`,
      "총세대수 30 미만이면서 주용도가 공동주택으로 기재되지 않은 건은 스냅샷에서 제외돼 있습니다.",
      "사업주체명·난방방식·분양구분은 관리공동형별개요에서 결합한 값인데, 그 대장이 준공 단지 위주라 결합률이 41% 안팎입니다. 비어 있는 것이 정상이며 '그런 값이 없다'는 뜻이 아닙니다.",
      "착공일은 준공 후에 채워지는 경향이 있습니다. 공정 역산은 사용검사예정일로 하세요.",
      "onlyOngoing은 사용검사일이 비고 사용검사예정일이 오늘 이후인 건만 남깁니다 — 사용검사일 공란만으로는 1990년대 준공 단지가 섞여 들어옵니다. 예정일조차 없어 진행 여부를 판정할 수 없는 건은 진행여부_판정불가_제외로 세어 밝힙니다.",
      "같은 지번·건물명이 대장 건별로 반복될 수 있어 지번+건물명으로 접었습니다(중복제거전_행수 참조).",
      "시공사는 이 대장에 없습니다 — search_housing_sales(청약홈)로 붙이세요.",
    ],
  };
}
...
export function registerHousingTools(server, z) {
  server.tool(
    "scan_national_housing_pipeline",
    "**전국 주택인허가 전수 스냅샷**에서 단지를 찾습니다. API 호출이 없어 즉시 응답하고, " +
      "지역을 지정하지 않아도 **전국이 한 번에** 검색됩니다.\n" +
      "★ search_housing_permits(API)는 조회 단위가 법정동이라 전국을 훑으려면 20,560개 법정동을 " +
      "순회해야 하고 일일 트래픽이 10,000건이라 하루에 불가능합니다. 이 도구가 그 한계를 없앱니다. " +
      "**전국 단위 질문(‘내년에 준공되는 1,000세대 이상 단지 다 뽑아줘’)은 반드시 이 도구를 쓰세요.**\n" +
      "★ **분양 여부와 무관한 전수**입니다. 지역주택조합 조합원분 전량·임대·후분양이 모두 들어 있어, " +
      "청약홈(search_housing_sales)에 빠지는 현장을 여기서 찾습니다. 분양구분(민간분양/민간임대/" +
      "공공임대/공공분양)으로 임대 현장만 따로 뽑을 수도 있습니다.\n" +
      "★ **시공사는 이 대장에 없습니다**(시행사만 있음). 시공사가 필요하면 search_housing_sales와 붙이세요.\n" +
      "★ 스냅샷이라 기준월 이후 신규 건은 없습니다. 최신 확인은 search_housing_permits로 그 지역만 재조회하세요.\n" +
      "★ 공정 역산은 착공일이 아니라 **사용검사예정일** 기준으로 하세요(착공일은 준공 후에 채워집니다).",
    {
      region: z.string().optional().describe("대지위치 부분검색(예: '경기도', '아산시', '서울특별시 강남구'). 생략하면 전국"),
      nameContains: z.string().optional().describe("건물명·단지명 부분검색. ★ 대장 명칭이 분양 브랜드명과 다른 경우가 많습니다"),
      developer: z.union([z.string(), z.array(z.string())]).optional().describe("사업주체(시행사) 부분검색. 배열 가능(OR)"),
      saleType: z.union([z.string(), z.array(z.string())]).optional().describe("분양구분 부분검색: 민간분양·민간임대·공공분양·공공임대(5년)·공공임대(10년)·공공임대(기타)·국민임대"),
      heating: z.string().optional().describe("난방방식 부분검색: 개별난방·지역난방·중앙집중난방"),
      minHouseholds: z.number().int().optional().describe("최소 총세대수"),
      maxHouseholds: z.number().int().optional().describe("최대 총세대수"),
      useInsptSchedFrom: z.string().optional().describe("사용검사예정일 시작 YYYYMMDD — 설비 영업 타깃을 잡는 핵심 조건"),
      ...
      onlyOngoing: z.boolean().optional().describe("진행 중 단지만 — 사용검사일이 없고 사용검사예정일이 오늘 이후인 건. ★ 사용검사일 공란만으로 거르면 1990년대 준공 단지가 섞입니다"),
      sort: z.string().optional().describe("정렬 컬럼 (기본 사용검사예정일)"),
      limit: z.number().int().min(1).max(1000).optional().describe("반환 단지 수 (기본 100)"),
    },
    async (args) => {
      const result = scanHousingPermitsBulk(args);
