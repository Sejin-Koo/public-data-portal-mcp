...
const AGENCY_CONFIGS = {
  ...
  dapa_overseas: {
    ...
    headers: {},
    // ★ 이 목록 응답의 필드는 seqn · anucType · anucTypeNm · bidNameKor · bidNameEng ·
    //   bidTo · infoAcquireFrom **일곱 개뿐이다**(2026-09-02 실측). 종전에는 여기서
    //   `it.orgPlaceNm`(나라)과 `it.bidFrom`을 읽고 있었는데 **목록에 그 필드 자체가 없어**
    //   늘 undefined가 나가고 있었다. 상세 오퍼레이션(getDCAIBidInfoDetail)에는 필드가
    //   존재하지만 값이 빈 문자열이라 거기서 채워올 수도 없다(seqn=1로 확인).
    //   → 존재하지 않는 값을 내보내지 않도록 두 필드를 뺐다. 데이터가 갱신되어 이 필드들이
    //     채워지면 그때 되살릴 것.
    //   ★ 대신 anucTypeNm을 넣어볼까 했으나 999건 중 985건이 "기타", 14건은 인코딩이 깨진
    //     "??"라 정보량이 없어 넣지 않았다(2026-09-02 실측).
    mapItem: (it) => ({
      title: it.bidNameKor,
    ...
    // ★★ 이 API는 **bidFrom/bidTo가 무시된다.** 실측(2026-09-02) — 파라미터를 아예 주지 않은
    //   것과 어떤 형식으로 준 것이 전부 같은 응답이었다(totalCount 7,205, 첫 seqn 1·2·3):
    //     파라미터 없음 / 20150601 / 2015.06.01 / 2015/06/01 / 201506 / 20150601000000
    //   ★ 형식 문제가 아니라는 것을 이렇게 확인했다 — 형식이 원인이면 최소 하나는 달라야 한다.
    //   ★ 공식 문서상 이 오퍼레이션의 파라미터는 serviceKey·pageNo·numOfRows·bidFrom·bidTo
    //     다섯 개뿐이므로, "다른 필터를 안 써서" 그런 것도 아니다.
    ...
    // ★★ 아래 note의 "멈춰 있다 / 비어 있다"는 모두 **2026-09-02 확인 시점의 상태**다. 데이터셋이
    //   되살아날 수 있으므로 단정으로 굳히지 말 것 — 이 소스를 다시 다루게 되면 그때 다시 잰다.
    note: "★ 아래는 모두 2026-09-02 확인 시점의 상태입니다(그 뒤로 다시 재지 않았으므로 지금도 같다고 단정하지 마세요). ①조회 구간이 반영되지 않습니다 — bidFrom/bidTo를 아예 주지 않은 것과 여섯 가지 형식으로 준 것이 전부 같은 응답이었습니다(totalCount 7,205 동일). ②데이터가 2019-02-26에서 멈춰 있습니다. ③나라(orgPlaceNm)·입찰시작일(bidFrom)·담당자·지역 필드는 목록에 없거나 값이 비어 있어 반환하지 않습니다. 따라서 최근 발주를 찾는 용도로는 쓸 수 없고 0건이 정상이며, 방위사업청 최근 발주는 dapa_bid(군수품조달정보)를 사용하세요. 갱신이 재개됐는지 궁금하면 이 소스를 한 번 조회해 infoAcquireFrom의 최대값을 보면 됩니다.",
  },
  dapa_bid: {
...
export async function scanAgencyBids({ agency, keywords, since, until } = {}) {
  ...
  // ★★ 날짜 필드가 빈 항목은 구간 판정이 불가하다. 버리지 않고 통과시키되 **몇 건인지 알린다.**
  //   실측(2026-09-02, 방위사업청 해외입찰): 1페이지 999건 중 5건이 infoAcquireFrom이 비어
  //   있었고, 이 5건은 어떤 조회 구간에서도 필터를 그냥 통과해 **2026년 8월을 조회해도 2015년
  //   항목이 섞여 나왔다.** 다른 세션 산출물이 "조회 창을 바꿔도 동일한 5건"이라고 관찰한 것이
  //   정확히 이 항목들이다.
  //   → 버리는 쪽(구간 밖으로 간주)은 날짜 필드가 부실한 소스에서 유효 공고까지 잃을 위험이
  //     있어 택하지 않았다. 입찰 정보는 놓치는 쪽이 섞이는 쪽보다 손해가 크다.
  const sinceYmd = toYYYYMMDD(range.since);
  const untilYmd = toYYYYMMDD(range.until);
  let 날짜미상 = 0;
  const dateFiltered = filtered.filter((it) => {
    const d = it.date ? String(it.date).replace(/[^0-9]/g, "") : "";
    if (d.length < 8) {
      날짜미상 += 1;
      return true;
    }
  ...
  return {
    ...
    유의사항: [
      ...
      ...(날짜미상
        ? [
            `★ 결과 ${dateFiltered.length}건 중 ${날짜미상}건은 원 자료에 날짜가 비어 있어 조회 구간(${range.since.slice(0, 8)}~${range.until.slice(0, 8)}) 안인지 판정할 수 없었습니다. 버리지 않고 포함했으므로 **구간 밖 자료가 섞여 있을 수 있습니다** — 개별 항목의 날짜를 확인하고, 구간 기준 건수·합계를 낼 때는 이 ${날짜미상}건을 빼고 세세요.`,
          ]
        : []),
