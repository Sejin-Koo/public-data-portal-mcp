...
// 명시적으로 디코딩한다.
export async function rawFetch(url, extraHeaders = {}) {
...
export async function scanNarajangteoProcurement({ keywords, since, until } = {}) {
  ...
  const byStage = {};
  for (const stageConfig of NARAJANGTEO_STAGES) {
    let items = [];
    for (const kw of kws) {
      const found = await fetchNarajangteoStage(stageConfig, kw, range.since, range.until, errors);
      items = items.concat(found);
    }
    items = dedupeBy(items, (it) => it.id);
    byStage[stageConfig.id] = items;
  }
