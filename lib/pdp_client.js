...
// 공공데이터포털 인증키.
//
// ★ 환경변수명이 DATA_PORTAL_KEY로 바뀌는 중이다(2026-08-26). 종전 이름
//   PUBLIC_DATA_PORTAL_KEY의 "PUBLIC"은 "공공데이터포털(Public Data Portal)"의 약자였는데,
//   Vercel은 `PUBLIC_` 접두어를 "브라우저에 노출할 공개 변수"로 해석해 Secret 타입 저장을
//   거부한다(Astro·Vite 등이 PUBLIC_* 를 클라이언트 번들에 심는 관례 때문). 이 서버에는
//   클라이언트 번들이 없어 실제 노출 경로는 없지만, 나중에 빌드 도구가 붙으면 진짜로 샐 수
//   있는 이름이라 접두어를 뗀다.
//
//   전환 중 서비스가 끊기지 않도록 **두 이름을 모두 인정**한다. Vercel에서 옛 변수를 지운
//   뒤에는 아래 폴백(|| process.env.PUBLIC_DATA_PORTAL_KEY)을 제거할 것.
export const SERVICE_KEY =
  process.env.DATA_PORTAL_KEY || process.env.PUBLIC_DATA_PORTAL_KEY || "";

/** 어느 환경변수명으로 키를 읽었는지 — 전환기 진단용. 키 값 자체는 절대 노출하지 않는다. */
export const SERVICE_KEY_SOURCE = process.env.DATA_PORTAL_KEY
  ? "DATA_PORTAL_KEY"
  : process.env.PUBLIC_DATA_PORTAL_KEY
    ? "PUBLIC_DATA_PORTAL_KEY(구 이름 — 전환 미완료)"
    : "미설정";
